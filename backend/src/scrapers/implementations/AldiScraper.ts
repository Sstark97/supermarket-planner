...
export class AldiScraper extends ScraperBase {
    ...
    protected async scrape(query: string): Promise<IProduct[]> {
        ...
        try {
            const response = await fetch('https://l9knu74io7-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20JavaScript%20(4.14.2)', {
                method: 'POST',
                headers: {
            ...
            return Promise.all(
                allHits.map(async (hit: any) => {
                    ...
                    return {
                        ...
                        url: hit.productUrl || (hit.url ? `https://www.aldi.es${hit.url}` : undefined),
