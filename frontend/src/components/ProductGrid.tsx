...
export async function ProductGrid({ query, category, supermarket, sortBy }: { query?: string, category?: string, supermarket?: string, sortBy?: string }) {
    ...
    return (
      <div className="py-20 text-center flex flex-col items-center justify-center bg-white rounded-3xl border border-dashed border-slate-200 m-8">
        ...
        <p className="text-slate-500 max-w-md">
          Prueba a buscar con otra palabra como "leche", "pan" o asegúrate de que el backend tiene datos en la base.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4 max-w-7xl mx-auto">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="animate-pulse bg-white p-4 rounded-2xl border border-slate-100 h-64 flex flex-col justify-between">
          <div>
