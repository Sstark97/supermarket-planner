        ...
        <div className="flex-1 overflow-y-auto p-5">
          {items.length === 0 ? (
            ...
            <ul className="space-y-4">
              {items.map((item) => (
                <li key={`${item.product.supermarket}-${item.product.id}`} className="flex gap-4 items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-medium text-slate-800 truncate">{item.product.name}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <SupermarketBadge supermarket={item.product.supermarket} />
